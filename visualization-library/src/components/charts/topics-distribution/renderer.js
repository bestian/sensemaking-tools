...
export function renderVisualization({
    ...
    const xAxisLabels = gridContainer
        ...
        .attr(
            "transform",
            `translate(0, ${dimensions.views["scatter"].innerHeight - margin / 2})`
        );
    ...
    const scatterLabels = scatterLabelsContainer
        .append("g")
        .attr("class", "labels");
    ...
    const elementsContainer = vizContainer
        .append("g")
    ...
    // Create label groups
    const clusterLabelContainer = clusterContainer
        .append("g")
    ...
    Array.from(groupedData.entries()).forEach(([topic, items], groupIndex) => {
        ...
        const groupX = col * dimensions.views["cluster"].groupWidth +
            dimensions.views["cluster"].groupPadding +
            dimensions.views["cluster"].bubbleSize / 2;
        const groupY = row * dimensions.views["cluster"].groupHeight +
            dimensions.views["cluster"].groupPadding;
