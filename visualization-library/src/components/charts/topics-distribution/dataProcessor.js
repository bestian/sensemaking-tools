...
export function processData(rawData, dimensions, getColor, visibleTopics = null) {
    ...
    const alignmentScale = d3
        .scaleLinear()
        .domain([1, 0])
        .range([0, views.scatter.innerHeight]);
    ...
    Array.from(topicMap.entries()).forEach(([topic, items], groupIndex) => {
        ...
        const groupX = col * clusterView.groupWidth + clusterView.groupPadding + clusterView.bubbleSize / 2;
        ...
        const pack = d3
            .pack()
            .size([clusterView.bubbleSize, clusterView.bubbleSize])
            .padding(0);

        // Generate initial node positions
        const initialNodes = pack(root).leaves().map((node) => ({
            x: node.x,
            y: node.y,
            name: node.data.name,
