 ...
 */
export function filterByTopic(data, topicFilter, getTopics = (d) => d.topics, getSubtopics = (d) => d.subtopics) {
    ...
    return data.filter((d) => {
        ...
        const topicPairs = topics.map((topic, index) => 
            `${topic.toLowerCase()}:${subtopics[index]?.toLowerCase() || ""}`
        );
        ...
        const matchesInclude = include.length === 0 || 
            include.some(filter => {
                ...
                return topicPairs.some(pair => {
                    ...
                    return (!filterSubtopic || subtopic === filterSubtopic) && 
                           topic === filterTopic;
                });
            });
