...
function csvTopicAlignmentSolid(data, topicFilter) {
    const groupLabels = [
        ...
        { key: 'uncategorized', label: 'Uncategorized', test: d => !d.isHighAlignment && !d.isLowAlignment && !d.isHighUncertainty },
    ];
    let csvRows = [
        ['topic', 'group', 'percentage of statements']
    ];
    if (topicFilter) {
...
function csvTopicAlignmentWaffle(data, topicFilter) {
    ...
    data.forEach(d => {
        ...
        d.topics.forEach((topic, i) => {
            ...
            let totalVotes = 0, agree = 0, disagree = 0, passed = 0;
            if (d.votes) {
...
function getProminentThemes(summaryData, topic, subtopic) {
    ...
    return prominent && prominent.text ? '"' + prominent.text.replace(/"/g, '""').replace(/\r?\n/g, ' ') + '"' : '';
}
...
function csvTopicsOverview(data, summaryData) {
    ...
    let csvRows = [
        ['topic', 'subtopic', 'statement count', 'prominent themes']
    ];
    const sortedKeys = sortTopicSubtopicKeys(Array.from(groupMap.keys()));
    sortedKeys.forEach(key => {
        const items = groupMap.get(key);
