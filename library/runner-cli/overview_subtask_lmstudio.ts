...
async function main(): Promise<void> {
  ...
  program
    .name("overview_subtask_lmstudio")
    .description("Run only the overview summarization subtask against a live LM Studio model.")
    .option("-m, --model <model>", "LM Studio model identifier.", "nvidia/nemotron-3-nano-4b")
    .option("--baseUrl <url>", "LM Studio OpenAI-compatible base URL.", "http://127.0.0.1:1234/v1")
    .option("--maxTokens <count>", "Maximum completion tokens per request.", "4096")
    ...
    .option(
      ...
      "tmp/overview-subtask-lmstudio.json"
    );

  program.parse(process.argv);
  const options = program.opts();

  const maxTokens = parsePositiveInt(options.maxTokens, "maxTokens");
