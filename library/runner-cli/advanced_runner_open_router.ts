...
// Advanced runner that produces structured report data using an OpenRouter model.
//
// Outputs three JSON files (matching advanced_runner_lmstudio.ts):
//  - <basename>-topic-stats.json
//  - <basename>-comments-with-scores.json
...
async function main(): Promise<void> {
  ...
  program
    ...
    .option(
      ...
      "Short context about the conversation to guide summarization."
    )
    .option(
      "-m, --model <model>",
      "OpenRouter model identifier (e.g. 'openai/gpt-oss-120b', 'anthropic/claude-3.5-sonnet')."
    )
    .option(
      "-k, --apiKey <key>",
      "OpenRouter API key. If omitted, falls back to OPENROUTER_API_KEY env var."
    )
  ...
  const { categorizedComments, summary } = await summarizeWithOpenRouter(
    comments,
    options.outputLang,
    apiKey,
    modelName,
  ...
  if (stats.getStatsByTopic().length === 0) {
    throw new Error("OpenRouter model returned no topics. Try rerunning with more context.");
  }
